...
def prep_ts_for_plotting(si, params):
    ...
    ## will look for default file name below, but can load ts with a different name using <plot:existing_ts>
        ...
        file_name = params['plot']['existing_ts']
        if filter_class:
            clean_class = filter_class.replace("'", "").replace('"', "")
            file_name = params['plot']['existing_ts'].replace('.csv',f'_{clean_class}.csv')
        ts_in = load_ts_from_file(file_name)
...
def plot_timeseries(params):
    ...
    def get_namedet(params):    
        ...
        if params['sample_model']['filter_class']:
            if isinstance(params['sample_model']['filter_class'],list):
                logger.info(f"filter class: {params['sample_model']['filter_class']}")
    ...
    def format_figure(params,fig,title,out_path,lines=None, labels=None):
        ...
        if params['plot']['legend'] and 'sub' not in params['plot']['legend']:
            ...
            fig.legend(unique_legend.values(), unique_legend.keys(), loc=params['plot']['legend'])
    ...
    def format_subplot(axs, params):
        ...
        if (params['plot']['legend']) and (params['plot']['legend'].startswith('sub')):
            ...
            axs.legend(unique_legend.values(), unique_legend.keys(), loc=legend_loc)
        return axs
    ...
    for inum, idx in enumerate(idx_bases):
        ...
        if len(idx_sm) > 0:
            ...
            fig, axs = plt.subplots(nrows=numrows, ncols=numcols, squeeze=False, figsize=(15, 5*numrows), 
                                    dpi=params['plot']['dpi'], layout="constrained")
            if params['plot']['multi_idx']:
                plot_multiidx = True
                break
            ...
            if params['plot']['multi_coords']:
                ...
                for p, pt in enumerate(tsr.columns.to_list()):   
                    ...
                    axs[row,col] = format_subplot(axs[inum,inum//2], params)
            lines, labels = axs[inum,inum//2].get_legend_handles_labels()
            params['feature_model']['spec_indices'] = all_sis
            fig_final = format_figure(params,fig,title,out_fig,lines,labels)
    ...
    if plot_multiidx:
        ...
        for inum, idxs in enumerate(smoothed_sis):
            if inum == 0:
                ## first smoothed ts is already calculated above
                ts = ts0
            else:
            ...
            for p, pt in enumerate(ts.columns.to_list()):
                row, col = get_row_col(p, numcols)
                axs[row, col].plot(ts.index, ts[pt], color=colors[inum], label=idxs)
            if f"{idxs.split('-')[0]}-raw" in raw_sis:
                ts = prep_ts_for_plotting(f"{idxs.split('-')[0]}-raw", params)
                for p, pt in enumerate(ts.columns.to_list()):
                    row, col = get_row_col(p, numcols)
                    axs[row, col].scatter(ts.index, ts[pt], color=colors[inum])
        ...
        for inum, idx in enumerate(idx_bases):
            ...
            for c, lcclass in enumerate(classes):
                ...
                if idx_sm:
                    params['feature_model']['spec_indices'] = idx
                    if inum == 0 and c == 0:
                        tss = ts0
                    else:
                        tss = prep_ts_for_plotting(idx_sm[0], params)
                            ...
                            for p, pt in enumerate(tss.columns.to_list()):
                                if (pt != 'ALL') and (pt !='std'):  ## these will only exist if <plot:multi_coords> = individual_w_med
                                    if len(classes) <= 1:
                                        ...
                                        elif ts0.shape[1] < 10:   ## only label pts if there is a manageable amount!
                ...
                if idx_raw:
                    ...
                    if inum == 0 and c == 0:
                        ...
                        tsr = prep_ts_for_plotting(f'{idx}-raw', params)
                    logger.debug(f": tsr shape: {tsr.shape}, columns found: {tsr.columns.to_list()}")
                    for p, pt in enumerate(tsr.columns.to_list()):
                        if len(classes) <= 1:
                            axs[row,col].scatter(tsr.index, tsr[pt], edgecolor='white', lw=0.1, s=10, color=colors[p], label=pt)
                        else:
